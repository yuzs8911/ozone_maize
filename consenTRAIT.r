...
#loading Newick tree (multitree)
tree_all = read.tree("bac_N240_sig_100boottree.newick",keep.multi = TRUE)
#Loading trait table w. no headers
tableinput = read.table("bac_N240_increase_traits.txt", sep = "\t", header=FALSE)
ncol(tableinput)
nrow(tableinput)
try_matrix<-as.data.frame(matrix(NA,ncol = 1000,nrow = nrow(tableinput)))
dim(try_matrix)
try_matrix[,1] <- tableinput[1:nrow(tableinput),1]
for (i in 1:999){
try_matrix[,i+1] <- sample(tableinput[1:nrow(tableinput),2])
}

table = try_matrix
#Starting script
Mean_all<-matrix(nrow=ncol(table)-1,ncol=101)
...
for (m in 1:length(tree_all)) {#loop through all trees
  ...
  for (j in 2:ncol(table)) {
    ...
    for (i in 1:length(subtree)){
      ...
      if (mean(table2[tip_names][,Trait],na.rm=TRUE) > 0.9) {#change this value if you want a new threshold
...
#output file
write.table(Mean_all,"TD.bac.N240.increase.observed.txt", sep = "\t")
